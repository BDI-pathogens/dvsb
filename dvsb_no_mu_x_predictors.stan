...
data {
  ...
  real p_pos_binary_effects_lower;
  real<lower = p_pos_binary_effects_lower> p_pos_binary_effects_upper;
...
parameters {
  ...
  vector<lower = p_pos_binary_effects_lower,
  upper = p_pos_binary_effects_upper>[num_p_pos_binary_pred_vars] p_pos_binary_effects;
}
...
transformed parameters{
  ...
  if (predict_p_pos || predict_p_pos_binary) {
    ...
    if (predict_p_pos_binary) {
      p_pos_per_sam_id +=
      design_matrix_p_pos_binary * p_pos_binary_effects;
    }
...
model {
  ...
  for (sam_id in 1:num_sam_id) {
    target += log_sum_exp(
      ...
      mu_pos, sd_pos_per_sam_id[sam_id]),
      p_neg_log_per_sam_id[sam_id] + normal_lpdf(xlog_sam[sam_id] |
      mu_neg, sd_neg_per_sam_id[sam_id]));
  }
  }
...
generated quantities {
  ...
  for (sam_id in 1:num_sam_id) {
    if (pos_sam_sim_unconditional[sam_id]) {
      ...
      normal_rng(mu_pos, sd_pos_per_sam_id[sam_id]);
    } else {
      xlog_sam_sim_unconditional[sam_id] = 
      normal_rng(mu_neg, sd_neg_per_sam_id[sam_id]);
    }
  }
  for (sam_rep in 1:num_sam_rep) {
    int plate = which_plate_sam[sam_rep];
  ...
  for (sam_id in 1:num_sam_id) {
    ...
    normal_lpdf(xlog_sam[sam_id] | mu_pos, sd_pos_per_sam_id[sam_id]);
    p_sam_is_pos[sam_id] = exp(p_log - log_sum_exp(p_log, p_neg_log_per_sam_id[sam_id] +
    normal_lpdf(xlog_sam[sam_id] | mu_neg, sd_neg_per_sam_id[sam_id])));
  }
