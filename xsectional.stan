...
data {
  ...
  vector[num_obs_con] y_con;
  real x_con[num_obs_con];
...
transformed parameters{
  ...
  vector[num_obs_con] y_con_mean_per_obs;
  ...
  for (con in 1:num_obs_con) {
    y_con_mean_per_obs[con] = 
    PL4(x_con[con], f_per_bat[which_bat_con[con]][1],
...
generated quantities {
  real y_con_sim[num_obs_con] = normal_rng(y_con_mean_per_obs, y_obs_sd);
}
